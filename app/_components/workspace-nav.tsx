...
type BudidayaTab = "overview" | "ponds" | "cycles" | "input" | "sampling" | "harvest" | "expenses";
type SalesTab =
  | "overview"
  | "leads"
  | "customers"
...
function WorkspaceTabs({ active, items, ariaLabel }: { active: string; items: NavItem[]; ariaLabel: string }) {
  return (
    <nav className="workspaceTabBar" aria-label={ariaLabel}>
      {items.map((item) => item.href ? (
        <Link className={`workspaceTab ${active === item.key ? "active" : ""}`} href={item.href} key={item.key}>
          <span>{item.label}</span>{item.hint ? <small>{item.hint}</small> : null}
        </Link>
      ) : (
        <span className="workspaceTab disabled" key={item.key} aria-disabled="true"><span>{item.label}</span>{item.hint ? <small>{item.hint}</small> : null}</span>
      ))}
    </nav>
  );
}
...
export function BudidayaWorkspaceNav({ active }: { active: BudidayaTab }) {
  return <WorkspaceTabs active={active} ariaLabel="Navigasi operasional budidaya" items={[
    { key: "overview", label: "Overview", href: "/budidaya" },
    { key: "ponds", label: "Kolam", href: "/ponds" },
    { key: "cycles", label: "Siklus", href: "/cycles" },
...
export function SalesWorkspaceNav({ active }: { active: SalesTab }) {
  return <WorkspaceTabs active={active} ariaLabel="Navigasi Sales CRM" items={[
...
export function UtilityWorkspaceNav({ active }: { active: UtilityTab }) {
  return <WorkspaceTabs active={active} ariaLabel="Navigasi riwayat dan laporan" items={[
