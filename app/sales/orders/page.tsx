...
export default async function SalesOrdersPage({
  ...
  try {
    [orders, options, opportunityWorkspace] = await Promise.all([
      getSalesOrders(),
      getSalesFormOptions(),
  ...
  return (
    <div className="opsPage crmWorkspacePage">
      <div className="workspaceHeadingRow">
        ...
        <div className="workspaceHeadingStats"><span><b>{orders.length}</b> total order</span><span><b>{openOrders}</b> masih terbuka</span></div>
      </div>
      ...
      <div className="workspaceSplit crmSplit">
        <form action={submitSalesOrder} className="workspaceCard operationalWorkspaceForm">
          ...
          <div className="formSectionBlock">
            ...
            <label><span>Opportunity (opsional)</span><select name="opportunityId" defaultValue={selectedOpportunity?.id ?? ""}><option value="">Order langsung tanpa opportunity</option>{openOpportunities.map((item) => <option value={item.id} key={item.id}>{item.title} · {item.customer.name}</option>)}</select></label>
            <div className="compactFieldGrid two">
              <label><span>Customer</span><select name="customerId" required defaultValue={selectedCustomerId}><option value="">Pilih customer</option>{options.customers.map((customer) => <option value={customer.id} key={customer.id}>{customer.name} · {customer.customerType}</option>)}</select></label>
              <label><span>Species / produk</span><select name="speciesId" required defaultValue={selectedSpeciesId}><option value="">Pilih species</option>{options.species.map((species) => <option value={species.id} key={species.id}>{species.commonName}</option>)}</select></label>
            </div>
          </div>
          ...
          <div className="formSectionBlock">
            <h3>Nilai order</h3>
            <div className="compactFieldGrid two">
              <label><span>Jumlah order</span><div className="compactUnitInput"><input name="quantityKg" type="number" min="0.001" step="0.001" required defaultValue={selectedOpportunity?.expectedQtyKg ? Number(selectedOpportunity.expectedQtyKg) : undefined} placeholder="100" /><b>kg</b></div></label>
              <label><span>Harga per kg</span><div className="compactUnitInput money"><b>Rp</b><input name="unitPricePerKg" type="number" min="1" step="1" required defaultValue={selectedOpportunity?.expectedPricePerKg ? Number(selectedOpportunity.expectedPricePerKg) : undefined} placeholder="23500" /></div></label>
            </div>
          </div>
        ...
        <section className="workspaceCard recordWorkspaceCard">
          ...
          <div className="recordTable orderRecordTable">
            <div className="recordTableHead"><span>Order</span><span>Nilai</span><span>Fulfillment</span><span>Finance</span><span>Status</span></div>
            {orders.length === 0 ? <div className="recordEmpty">Belum ada sales order.</div> : orders.map((order) => {
              ...
              const invoiced = order.invoices.filter((invoice) => invoice.status !== "VOID").reduce((sum, invoice) => sum + Number(invoice.totalAmount), 0);
              const paid = order.invoices.reduce((sum, invoice) => sum + invoice.payments.reduce((inner, payment) => inner + Number(payment.amount), 0), 0);
              return <article className="recordTableRow" key={order.id}>
