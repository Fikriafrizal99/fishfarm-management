  ...
  if (!dashboard) {
    return (
      <div className="opsPage">
        <section className="workspaceCard emptyState">
    ...
    <div className="opsPage crmWorkspacePage salesOpsPage">
      <div className="workspaceHeadingRow">
        ...
        <div className="quickActions">
          ...
          <Link className="actionButton primary" href="/sales/orders">+ Order</Link>
        </div>
      </div>
      ...
      <div className="salesDashboardGrid">
        <section className="workspaceCard">
          <div className="sectionHeaderInline">
            <div><p className="eyebrow dark">Pipeline</p><h2>Lead yang perlu ditindaklanjuti</h2></div>
            <Link href="/sales/leads">Semua →</Link>
          </div>
            ...
            {dashboard.upcomingLeads.length === 0 ? (
              ...
              <div className="crmRow" key={lead.id}>
                <div>
                  ...
                  <span>{lead.expectedDemandKg === null ? "Qty belum ditentukan" : `${number1.format(lead.expectedDemandKg)} kg`}{lead.expectedPricePerKg === null ? "" : ` · ${currency.format(lead.expectedPricePerKg)}/kg`}</span>
                </div>
                <div className="crmRowRight"><span className="badge warning">{lead.status}</span><small>{lead.nextFollowUpAt ? dateFormatter.format(lead.nextFollowUpAt) : "No follow-up"}</small></div>
              </div>
            ))}
          </div>
        </section>
        ...
        <section className="workspaceCard">
          <div className="sectionHeaderInline">
            <div><p className="eyebrow dark">Order Book</p><h2>Order aktif</h2></div>
            <Link href="/sales/orders">Semua →</Link>
          </div>
          <div className="crmList">
            {dashboard.activeOrders.length === 0 ? (
              ...
              <div className="crmRow" key={order.id}>
                <div><strong>{order.orderNumber} · {order.customerName}</strong><span>{number1.format(order.quantityKg)} kg · {currency.format(order.orderValue)}</span></div>
                <div className="crmRowRight"><span className="badge good">{order.status}</span><small>{number1.format(order.allocatedKg)} kg allocated</small></div>
              </div>
            ))}
          </div>
        </section>
      ...
      <section className="workspaceCard moduleMenuCard">
        <div className="sectionHeaderInline">
          <div><p className="eyebrow dark">Fulfillment Bridge</p><h2>Harvest Inventory</h2></div>
          <Link href="/sales/fulfillment">Buka fulfillment →</Link>
        </div>
        ...
        <div className="crmList">
          {dashboard.inventoryLots.length === 0 ? (
            <div className="emptyInline">Belum ada HarvestLot. Order tetap dapat dicatat dan dialokasikan setelah panen tersedia.</div>
          ) : dashboard.inventoryLots.map((lot) => (
            <div className="crmRow" key={lot.id}>
              <div><strong>{lot.lotCode} · {lot.species}</strong><span>{lot.pondCode} · panen {dateFormatter.format(lot.harvestedAt)}</span></div>
              <div className="crmRowRight"><strong>{number1.format(lot.availableKg)} kg available</strong><small>{number1.format(lot.allocatedKg)} / {number1.format(lot.quantityKg)} kg allocated</small></div>
            </div>
          ))}
        </div>
      </section>
